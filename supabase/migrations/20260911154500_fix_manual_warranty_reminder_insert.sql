-- Avoid a collision between the function's RETURNS TABLE column names and the
-- reminder uniqueness target. The protection row lock keeps this insert safe
-- against concurrent duration updates for the same protection.
